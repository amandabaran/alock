...
# for num_nodes in 20 
# do
#   for num_clients in 180
#     do 
#       for max in 10
#       do
#         for budget in 2 5 10 20 30 50
#         do 
...
do
  for num_threads in 2 4 8 16 20 30 
    do 
      for max in 10
      do
        for local_p in 1.0 0.75 0.5 0.25
        do 
          num_clients=$((num_threads * num_nodes))
          bazel run //alock/benchmark/one_lock:launch -- -n ${nodefile} -C ${num_clients} --nodes=${num_nodes} --threads=${num_threads} --budget=5 --max_key=${max} --p_local=${local_p} --lock_type=${lock} --ssh_user=adb321 --think_ns=0 --runtime=10 --remote_save_dir=${save_dir} --log_level=${log_level} --dry_run=False
        done
      done
    done
done
