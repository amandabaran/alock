...
# Experiments needed to create Figure 1 - RDMA loopback scalability plot

num_nodes=1

for num_threads in 4 8 12 16 24 32 64 96 108 120 132 148 156 164 172
do 
  for max in 1000
  do
    num_clients=$(( num_threads*num_nodes ))
    bazel run //alock/benchmark/one_lock:launch -- -n ${nodefile} -C ${num_clients} --nodes=${num_nodes} --ssh_user=adb321 --lock_type=${lock} --think_ns=0 --runtime=10 --remote_save_dir=${save_dir} --log_level=${log_level} --threads=${num_threads} --max_key=${max} --dry_run=False
  done
done
