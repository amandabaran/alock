...
for i in `seq 0 ${last_valid_index}`
do
    echo "screen -t node${i} ssh ${user}@${machines[$i]}.${domain} ./${exename} --node_count ${num_nodes} --node_id ${i} --runtime ${runtime} --op_count ${op_count} --min_key ${min_key} --max_key ${max_key} --region_size ${region_size} --thread_count ${thread_count} --qp_max ${qp_max} --p_local ${p_local} --local_budget ${local_budget} --remote_budget ${remote_budget}; bash" >> ${screenrc_run}
